...
const solutionC = document.querySelector('.line');
const ctx = canvas.getContext('2d');
const ctx1 = solutionC.getContext('2d');
...
let CANVAS_WIDTH = canvas.width = 484; //height in width canvasa
let CANVAS_HEIGHT = canvas.height = 484;
let scale = CANVAS_HEIGHT / 484;
let OFFSET = 1.5;
let DIFF = 16;
let SPRITE_SIZE = 32;
...
function drawSolution(points, speed) {
    ...
    drawInterval = setInterval(() => {
        ...
        ctx1.lineTo(points[index][0], points[index][1]);
        ctx1.stroke();
        index++;
    }, speed);
...
function reset(points, speed) {
    ...
    resetInterval = setInterval(() => {
        ...
        if (points[idx][1] === points[idx + 1][1]) {
            ctx1.clearRect(
                Math.min(points[idx][0], points[idx + 1][0]) - offset, 
                points[idx][1] - offset, 
                Math.abs(points[idx + 1][0] - points[idx][0]) + width, 
                width
            );
        } else if (points[idx][0] === points[idx + 1][0]) {
            ctx1.clearRect(
                points[idx][0] - offset, 
                Math.min(points[idx][1], points[idx + 1][1]) - offset, 
                width, 
                Math.abs(points[idx + 1][1] - points[idx][1]) + width
            );
        }
